...
fun getDisallowInstallUnknownSourcesErrorMessage(context: Context): String {
  ...
  return if (
    SDK_INT >= 29 && userManager.hasUserRestriction(DISALLOW_INSTALL_UNKNOWN_SOURCES_GLOBALLY)
  ) {
...
fun AddRepoErrorInvalidFingerprintPreview() {
  FDroidContent { AddRepoErrorScreen(AddRepoError(INVALID_FINGERPRINT)) }
}

@Preview
@Composable
fun AddRepoErrorIoErrorPreview() {
  FDroidContent { AddRepoErrorScreen(AddRepoError(IO_ERROR, IOException("foo bar"))) }
}

@Preview
@Composable
fun AddRepoErrorInvalidIndexPreview() {
  FDroidContent { AddRepoErrorScreen(AddRepoError(INVALID_INDEX, RuntimeException("foo bar"))) }
}

@Preview
@Composable
fun AddRepoErrorUnknownSourcesPreview() {
  FDroidContent { AddRepoErrorScreen(AddRepoError(UNKNOWN_SOURCES_DISALLOWED)) }
}

@Preview
@Composable
fun AddRepoErrorArchivePreview() {
  FDroidContent { AddRepoErrorScreen(AddRepoError(IS_ARCHIVE_REPO)) }
}
