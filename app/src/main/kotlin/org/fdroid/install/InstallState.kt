...
sealed class InstallState(val showProgress: Boolean) {
  ...
  /**
   * Used for our own app which will be updated last, so this is waiting for all other updates to
   * complete.
   */
  data class Waiting(
  ...
  data class Downloading(
    ...
    val progress: Float
      get() = downloadedBytes / totalBytes.toFloat()
  }
...
sealed class InstallConfirmationState() : InstallStateWithInfo(true) {
  ...
  /**
   * The epoch time in milliseconds when this state was created. This is used to get a stable
   * ordering on apps that require user confirmation. The reason this is needed is that we can only
   * show a single confirmation dialog at a time. If we show more than one, the second one gets
   * silently swallowed by the system and we don't receive any feedback, so installation process of
   * several apps gets stuck.
   */
