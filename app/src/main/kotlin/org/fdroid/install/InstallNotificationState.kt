...
data class InstallNotificationState(
  ...
  /**
   * Returns true if there are apps that have an installation in progress which could be waiting for
   * user confirmation or downloading, or waiting for system installer.
   */
  val isInProgress: Boolean = apps.any { it.category != AppStateCategory.INSTALLED }

  /**
   * Returns true if there is at least one app either downloading for actually installing. If there
   * are only apps that have been installed already or are waiting for user confirmation, this will
   * return false.
   */
  ...
  val numInstalled: Int
    get() = apps.count { it.category == AppStateCategory.INSTALLED }
  ...
  fun getTitle(context: Context): String {
    ...
    val installTitle = context.resources.getQuantityString(titleRes, numActiveApps, numActiveApps)
