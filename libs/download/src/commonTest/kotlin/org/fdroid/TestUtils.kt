...
fun runSuspend(block: suspend () -> Unit) = runBlocking { block() }

fun HttpRequestData.getByteRangeFrom(): Int {
  val (fromStr, endStr) =
    (headers[Range] ?: fail("No Range header")).replace("bytes=", "").split('-')
  assertEquals("", endStr)
  return fromStr.toIntOrNull() ?: fail("No valid content range ${headers[Range]}")
}

fun get(mockEngine: MockEngine) =
...
/**
 * This class isn't reliable and produces flaky tests. It doesn't seem to be possible to mock failed
 * HTTP downloads where partial data gets transferred.
 */
...
internal class TestByteReadChannel(private val buffer: Buffer) : ByteReadChannel {
  @InternalAPI override val readBuffer: Source = buffer
