...
internal class HttpManagerTest {
  ...
  fun testResumeDownload() = runSuspend {
    ...
    assertFailsWith<NoResumeException> { httpManager.getBytes(downloadRequest, skipBytes.toLong()) }
  }
  ...
  fun testResumeDownloadWhenMirrorFailOver() = runSuspend {
    ...
    val sink = Buffer()
    httpManager.get(downloadRequest) { bytes, numTotalBytes -> sink.write(bytes) }
  ...
  fun testMirrorFallback() = runSuspend {
    ...
    assertFailsWith<ServerResponseException> { httpManager.getBytes(downloadRequest) }
  ...
  fun testNoMoreMirrorsWhenForbiddenWithCredentials() = runSuspend {
    ...
    assertNull(httpManager.head(downloadRequest))
    val e = assertFailsWith<ClientRequestException> { httpManager.getBytes(downloadRequest) }
  ...
  fun testNoMoreMirrorsWhenRepoDownloadNotFound() = runSuspend {
    ...
    assertFailsWith<NotFoundException> { httpManager.head(downloadRequest) }
    val e = assertFailsWith<ClientRequestException> { httpManager.getBytes(downloadRequest) }
  ...
  fun testNoRedirect() = runSuspend {
    ...
    assertNull(httpManager.head(downloadRequest))
    assertFailsWith<RedirectResponseException> { httpManager.getBytes(downloadRequest) }
