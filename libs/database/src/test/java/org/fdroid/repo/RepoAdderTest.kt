...
internal class RepoAdderTest {
  ...
  fun testDisallowInstallUnknownSources() = runTest {
    ...
    val repoAdder =
      RepoAdder(context, db, tempFileProvider, downloaderFactory, httpManager, compatibilityChecker)

    every { context.getSystemService(UserManager::class.java) } returns userManager
    every { userManager.hasUserRestriction(DISALLOW_INSTALL_UNKNOWN_SOURCES) } returns true
  ...
  private suspend fun testAddingMinRepoInt(url: String, expectedResult: FetchResult) {
  ...
  fun testAddingMidRepoByOfficialMirror() = runTest {
    ...
    verify(exactly = 0) { repoDao.updateUserMirrors(42L, listOf(url)) }
  }
  ...
  fun testAddingUserMirrorForExistingMinRepo() = runTest {
    ...
    val existingRepo =
      Repository(
        repository =
          TestDataMinV2.repo.toCoreRepository(
            repoId = 42L,
            version = 1337L,
    ...
    every { db.runInTransaction(capture(transactionSlot)) } answers
      {
        transactionSlot.captured.call()
      }
    ...
    verify(exactly = 1) { repoDao.updateUserMirrors(42L, listOf(url.trimEnd('/'))) }
  }
  ...
  private suspend fun testRepoAlreadyExists(
    ...
    // The "normalized" URL that the HTTP stack will end up requesting (i.e., without
  ...
  fun testParsingThrowsSerializationException() = runTest {
    ...
    every {
      downloaderFactory.create(
        repo = match { it.address == urlTrimmed && it.formatVersion == IndexFormatVersion.TWO },
    ...
    coEvery {
      httpManager.getDigestInputStream(
        match {
  ...
  fun testWrongFingerprint() = runTest {
    ...
    every {
      downloaderFactory.create(
        repo =
          match { it.address == url.trimEnd('/') && it.formatVersion == IndexFormatVersion.TWO },
        uri = Uri.parse(url + "entry.jar"),
        indexFile = any(),
        destFile = jarFile,
      )
  ...
  private suspend fun testMinRepoPreview(
    ...
    val jarFile = folder.newFile()
    val repoV2 = RepoV2(address = "https://briarproject.org/fdroid/repo", timestamp = 42L)
    ...
    every {
      downloaderFactory.create(
        repo = match { it.address == repoAddress && it.formatVersion == IndexFormatVersion.TWO },
  ...
  fun testKnownFingerprintIsAccepted() = runTest {
    ...
    val jarFile = folder.newFile()
    val repoV2 = RepoV2(address = repoAddress, timestamp = 42L)
    val indexV2 = IndexV2(repo = repoV2)
    val index = json.encodeToString(IndexV2.serializer(), indexV2).toByteArray()
    val indexStream = DigestInputStream(ByteArrayInputStream(index), digest)
    ...
    every {
      downloaderFactory.create(
        repo =
          match {
            it.address == "https://example.org/repo" && it.formatVersion == IndexFormatVersion.TWO
          },
  ...
  fun testFallbackToV1() = runTest {
    ...
    every {
      downloaderFactory.create(
        repo = match { it.address == urlTrimmed && it.formatVersion == IndexFormatVersion.ONE },
  ...
  private fun mockRepoDownload(
    ...
    every {
      downloaderFactory.create(
        repo =
          match { it.address == downloadUrlTrimmed && it.formatVersion == IndexFormatVersion.TWO },
    ...
    every { digest.digest() } returns digestHex.decodeHex()
  }
  ...
  private suspend fun expectMinRepoPreview(
    ...
    expectRepoPreview(repoName, url, expectedFetchResult, TestDataMinV2.repo) {
      val state = awaitItem()
  ...
  private suspend fun expectMidRepoPreview(
    ...
    expectRepoPreview(repoName, url, expectedFetchResult, TestDataMidV2.repo) {
      val state = awaitItem()
      assertIs<Fetching>(state)
