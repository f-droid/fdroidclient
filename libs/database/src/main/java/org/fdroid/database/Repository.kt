...
internal constructor(
  ...
  @Relation(parentColumn = "repoId", entityColumn = "repoId") internal val mirrors: List<Mirror>,
  ...
  public val webBaseUrl: String?
    get() = repository.webBaseUrl

  public val timestamp: Long
    get() = repository.timestamp

  public val version: Long
    get() = repository.version ?: 0

  public val formatVersion: IndexFormatVersion?
    get() = repository.formatVersion

  public val certificate: String
    get() = repository.certificate

  /**
   * True if this repository is an archive repo. It is suggested to not show archive repos in the
   * list of repos in the UI.
   */
  ...
  public val weight: Int
    get() = preferences.weight

  public val enabled: Boolean
    get() = preferences.enabled

  public val lastUpdated: Long?
    get() = preferences.lastUpdated

  public val userMirrors: List<String>
    get() = preferences.userMirrors ?: emptyList()

  public val disabledMirrors: List<String>
    get() = preferences.disabledMirrors ?: emptyList()
  ...
  /**
   * The fingerprint for the [certificate]. This gets calculated on first call and is an expensive
   * operation. Subsequent calls re-use the
   */
  @delegate:Ignore public val fingerprint: String by lazy { getFingerprint(certificate) }

  /** Returns official and user-added mirrors without the [disabledMirrors]. */
  public fun getMirrors(): List<org.fdroid.download.Mirror> {
    return getAllMirrors(true)
      .filter { !disabledMirrors.contains(it.baseUrl) }
  ...
  public fun getAllMirrors(includeUserMirrors: Boolean = true): List<org.fdroid.download.Mirror> {
    val all =
      mirrors.map { it.toDownloadMirror() } +
        if (includeUserMirrors) userMirrors.map { org.fdroid.download.Mirror(it) } else emptyList()
    ...
    return if (hasCanonicalMirror) all
    else all.toMutableList().apply { add(0, org.fdroid.download.Mirror(address)) }
  }
  ...
  public val errorCount: Int
    get() = preferences.errorCount

  public val lastError: String?
    get() = preferences.lastError
}

// Dummy repo to use in Compose Previews and in tests
@Deprecated("Will be removed in future version")
public val DUMMY_TEST_REPO: Repository =
  Repository(
    repoId = 1L,
...
internal fun MirrorV2.toMirror(repoId: Long) =
  Mirror(
    repoId = repoId,
    url = url,
    countryCode = countryCode,
...
internal data class RepositoryPreferences(
  ...
  @ColumnInfo(defaultValue = "0") val errorCount: Int = 0,
