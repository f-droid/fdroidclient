...
constructor(
   ...
   *   either via [AppPrefs.preferredRepoId] or [Repository.weight] will not be returned. If false,
   *   updates from all enabled repositories will be considered and the one with the highest version
   *   code returned.
   */
  @JvmOverloads
  public fun getUpdatableApps(
    ...
    versionDao.getVersions(packageNames).forEach { version ->
      val preferredRepoId =
        preferredRepos[version.packageName]
  ...
  /**
   * Returns an [AppVersion] for the given [packageName] that is an update or new install or null if
   * there is none.
   *
   * @param releaseChannels optional list of release channels to consider on top of stable. If this
   *   is null or empty, only versions without channel (stable) will be considered.
   * @param onlyFromPreferredRepo if true a version from a repository that is not preferred, either
   *   via [AppPrefs.preferredRepoId] or [Repository.weight] will not be returned. If false,
   *   versions from all enabled repositories will be considered.
   */
