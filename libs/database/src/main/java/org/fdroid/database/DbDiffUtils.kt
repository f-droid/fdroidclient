...
internal object DbDiffUtils {
  ...
  /**
   * Applies the diff from the given [jsonObject] identified by the given [jsonObjectKey] to
   * [itemList] and updates the DB as needed.
   *
    ...
    if (jsonObject[jsonObjectKey] == JsonNull) {
      ...
      obj.entries.forEach { (key, value) ->
        if (value is JsonNull) {
          ...
          if (item == null) {
            val itemToInsert = ReflectionDiffer.applyDiff(newItem(key), value.jsonObject)
  ...
  /**
   * Applies a list diff from a map of lists. The map is identified by the given [jsonObjectKey] in
   * the given [jsonObject]. The diff is applied for each key by replacing the existing list using
   * [deleteList] and [insertNewList].
   *
  ...
  /**
   * Applies the list diff from the given [jsonObject] identified by the given [jsonObjectKey] by
   * replacing an existing list using [deleteList] and [insertNewList].
   *
   * @param listParser returns a list of [T] from the given [JsonArray].
   */
  @Throws(SerializationException::class)
