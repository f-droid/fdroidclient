...
/**
 * The detailed metadata for an app. Almost all fields are optional. This largely represents
 * [MetadataV2] in a database table.
 */
@Entity(
...
public data class AppMetadata(
  ...
  /**
   * Whether the app is compatible with the current device. This value will be computed and is
   * always false until that happened. So to always get correct data, this MUST happen within the
   * same transaction that adds the [AppMetadata].
   */
...
/**
 * Introduce zero whitespace for CJK (Chinese, Japanese, Korean) languages. This is needed, because
...
/**
 * A class to represent all data of an App. It combines the metadata and localized filed such as
...
internal constructor(
  ...
  public val authorName: String?
    get() = metadata.authorName

  internal val icon: LocalizedFileV2?
    get() = getLocalizedFile("icon")

  internal val featureGraphic: LocalizedFileV2?
    get() = getLocalizedFile("featureGraphic")

  internal val promoGraphic: LocalizedFileV2?
    get() = getLocalizedFile("promoGraphic")

  internal val tvBanner: LocalizedFileV2?
    get() = getLocalizedFile("tvBanner")

  internal val screenshots: Screenshots?
    get() =
      if (localizedFileLists.isNullOrEmpty()) null
      else
        Screenshots(
  ...
  public fun getTvBanner(localeList: LocaleListCompat): FileV2? = tvBanner.getBestLocale(localeList)
 ...
 * without going into all details that get presented on a dedicated screen. The reduced data
 * footprint helps with fast loading many items at once.
 *
...
internal constructor(
  ...
  @ColumnInfo(name = "name") internal val internalName: LocalizedTextV2? = null,
  @ColumnInfo(name = "summary") internal val internalSummary: LocalizedTextV2? = null,
  public val categories: List<String>? = null,
  internal val antiFeatures: Map<String, LocalizedTextV2>? = null,
  @Relation(parentColumn = "packageName", entityColumn = "packageName")
  internal val localizedIcon: List<LocalizedIcon>? = null,
  /** If true, this app has at least one version that is compatible with this device. */
  public val isCompatible: Boolean,
) : MinimalApp {
  ...
  public val antiFeatureKeys: List<String>
    get() = antiFeatures?.map { it.key } ?: emptyList()
}

/**
 * Similar to [AppOverviewItem], this is a lightweight version of [App] meant to show a list of
 * apps.
 *
 * There is additional information about [installedVersionCode] and [installedVersionName] as well
 * as [isCompatible].
 *
 * It includes [antiFeatureKeys] of the highest version, so some clients can apply filters to them.
 */
@ConsistentCopyVisibility
public data class AppListItem
internal constructor(
  public override val repoId: Long,
  public override val packageName: String,
  @ColumnInfo(name = "localizedName") public override val name: String? = null,
  @ColumnInfo(name = "localizedSummary") public override val summary: String? = null,
  public val lastUpdated: Long,
  public val categories: List<String>? = null,
  ...
  @get:Ignore public val installedVersionName: String? = null,
  /** The version code of the installed version, null if this app is not installed. */
  @get:Ignore public val installedVersionCode: Long? = null,
) : MinimalApp {
  @delegate:Ignore
  private val antiFeaturesDecoded by lazy { fromStringToMapOfLocalizedTextV2(antiFeatures) }
...
internal constructor(
  ...
  /**
   * If true, this is not necessarily an update (contrary to the class name), but an app with the
   * `KnownVuln` anti-feature.
   */
  ...
  public override fun getIcon(localeList: LocaleListCompat): FileV2? {
    return localizedIcon
      ?.filter { icon -> icon.repoId == update.repoId }
      ?.toLocalizedFileV2()
      .getBestLocale(localeList)
  }
}
