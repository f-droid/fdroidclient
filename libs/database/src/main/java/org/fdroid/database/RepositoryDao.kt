...
public interface RepositoryDao {
  ...
  /**
   * Enables or disables the repository with the given [repoId]. Data from disabled repositories is
   * ignored in many queries.
   */
  public fun setRepositoryEnabled(repoId: Long, enabled: Boolean)

  /**
   * Updates the user-defined mirrors of the repository with the given [repoId]. The existing
   * mirrors get overwritten with the given [mirrors].
   */
  public fun updateUserMirrors(repoId: Long, mirrors: List<String>)

  /**
   * Updates the username and password (for basic authentication) of the repository with the given
   * [repoId]. The existing username and password get overwritten with the given [username] and
   * [password].
   */
  public fun updateUsernameAndPassword(repoId: Long, username: String?, password: String?)

  /**
   * Updates the disabled mirrors of the repository with the given [repoId]. The existing disabled
   * mirrors get overwritten with the given [disabledMirrors].
   */
  ...
  /**
   * Force a checkpoint on the SQLite WAL such that the file size gets reduced. Blocks until
   * concurrent reads have finished. Useful to call after large inserts (repo update)
   *
   * @see <a
   *   href="https://www.sqlite.org/wal.html#avoiding_excessively_large_wal_files">https://www.sqlite.org/wal.html#avoiding_excessively_large_wal_files</a>
   */
...
internal interface RepositoryDaoInt : RepositoryDao {
  ...
  @Insert(onConflict = REPLACE) fun insertOrReplace(repository: CoreRepository): Long

  @Update fun update(repository: CoreRepository)

  @Insert(onConflict = REPLACE) fun insertMirrors(mirrors: List<Mirror>)

  @Insert(onConflict = REPLACE) fun insertAntiFeatures(repoFeature: List<AntiFeature>)

  @Insert(onConflict = REPLACE) fun insertCategories(repoFeature: List<Category>)

  @Insert(onConflict = REPLACE) fun insertReleaseChannels(repoFeature: List<ReleaseChannel>)

  @Insert(onConflict = REPLACE) fun insert(repositoryPreferences: RepositoryPreferences)
  ...
  /**
   * Updates an existing repo with new data from a full index update. Call [clear] first to ensure
   * old data was removed.
   */
  @Transaction
  fun update(repoId: Long, repository: RepoV2, version: Long, formatVersion: IndexFormatVersion) {
  ...
  @Update fun updateRepository(repo: CoreRepository): Int

  @Update fun updateRepositoryPreferences(preferences: RepositoryPreferences)
  ...
  fun updateRepository(repoId: Long, version: Long, jsonObject: JsonObject) {
    ...
    diffAndUpdateListTable(
      ...
      listParser = { mirrorArray ->
        json.decodeFromJsonElement<List<MirrorV2>>(mirrorArray).map { it.toMirror(repoId) }
      },
  ...
  /**
   * Changes repository weights/priorities that determine list order and preferred repositories. The
   * lower a repository is in the list, the lower is its priority. If an app is in more than one
   * repo, by default, the repo higher in the list wins.
   *
   * @param repoToReorder this repository will change its position in the list.
   * @param repoTarget the repository in which place the [repoToReorder] shall be moved. If our list
   *   is [ A B C D ] and we call reorderRepositories(B, D), then the new list will be [ A C D B ].
   * @throws IllegalArgumentException if one of the repos is an archive repo. Those are expected to
   *   be tied to their main repo one down the list and are moved automatically when their main repo
   *   moves.
   */
  @Transaction
  ...
  @Query("DELETE FROM ${CoreRepository.TABLE}") fun deleteAllCoreRepositories()

  @Query("DELETE FROM ${RepositoryPreferences.TABLE}") fun deleteAllRepositoryPreferences()

  /** Used for diffing. */
  @Query("DELETE FROM ${Mirror.TABLE} WHERE repoId = :repoId") fun deleteMirrors(repoId: Long)
  ...
  /** Used for diffing. */
  @Query("DELETE FROM ${Category.TABLE} WHERE repoId = :repoId") fun deleteCategories(repoId: Long)
  ...
  /**
   * Resets timestamps for *all* repos in the database. This will use a full index instead of diffs
   * when updating the repository via [IndexV2Updater].
   */
  @Query("UPDATE ${CoreRepository.TABLE} SET timestamp = -1") fun resetTimestamps()

  /**
   * Resets ETags for *all* repos in the database. This will use cause a full index update when
   * updating the repository via [IndexV1Updater].
   */
  @Query("UPDATE ${RepositoryPreferences.TABLE} SET lastETag = NULL") fun resetETags()

  /**
   * Use when replacing an existing repo with a full index. This removes all existing index data
   * associated with this repo from the database, but does not touch repository preferences.
   *
  ...
  @VisibleForTesting @Query("SELECT COUNT(*) FROM ${Mirror.TABLE}") fun countMirrors(): Int

  @VisibleForTesting
  @Query("SELECT COUNT(*) FROM ${AntiFeature.TABLE}")
  fun countAntiFeatures(): Int

  @VisibleForTesting @Query("SELECT COUNT(*) FROM ${Category.TABLE}") fun countCategories(): Int
  ...
  @RawQuery fun rawCheckpoint(supportSQLiteQuery: SupportSQLiteQuery): Int
}
