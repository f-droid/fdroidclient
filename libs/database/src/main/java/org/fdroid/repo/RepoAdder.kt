...
public class Added(public val repo: Repository, public val updateResult: IndexUpdateResult?) :
  AddRepoState()
...
internal class RepoAdder(
  ...
  internal fun fetchRepository(url: String, proxy: Proxy?) {
    fetchJob = GlobalScope.launch(coroutineContext) { fetchRepositoryInt(url, proxy) }
  }
  ...
  internal suspend fun fetchRepositoryInt(url: String, proxy: Proxy? = null) {
    ...
    if (nUri.uri.scheme !in listOf("content", "file") && isInvalidHttpUrl(nUri.uri.toString())) {
      ...
      object : RepoPreviewReceiver {
        override fun onRepoReceived(repo: Repository) {
          ...
          if (repo.address in knownRepos) {
            ...
            if (knownFingerprint != repo.fingerprint)
  ...
  private suspend fun fetchRepo(
    ...
    return try {
      ...
      val repoFetcher =
        RepoV2Fetcher(tempFileProvider, downloaderFactory, httpManager, repoUriBuilder, proxy)
  ...
  private fun getFetchResult(fetchUrlIn: String, fetchedRepo: Repository): FetchResult {
    ...
    val isNewMirror =
      existingRepo.getAllMirrors().find { fetchUrl == it.url.toString().trimEnd('/') } == null
  ...
  internal fun addFetchedRepository(): Repository? {
    ...
    val fetchResult =
      state.fetchResult ?: throw IllegalStateException("No fetchResult: ${addRepoState.value}")

    var indexUpdateResult: IndexUpdateResult? = null
    val modifiedRepo: Repository =
      when (fetchResult) {
        ...
        is FetchResult.IsNewRepoAndNewMirror -> {
        ...
        is FetchResult.IsNewMirror -> {
          ...
          db.runInTransaction<Repository> {
            val existingRepo = repositoryDao.getRepository(repoId) ?: error("No repo with $repoId")
            val userMirrors = existingRepo.userMirrors.toMutableList().apply { add(state.fetchUrl) }
            repositoryDao.updateUserMirrors(repoId, userMirrors)
            existingRepo
          }
        }
  ...
  private fun hasDisallowInstallUnknownSources(context: Context): Boolean {
    val userManager =
      getSystemService(context, UserManager::class.java) ?: error("No UserManager available.")
