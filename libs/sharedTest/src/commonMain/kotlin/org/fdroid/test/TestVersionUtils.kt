...
object TestVersionUtils {
  ...
  fun getRandomPackageVersionV2(
    ...
    signer: SignerV2? =
      SignerV2(getRandomList(Random.nextInt(1, 3)) { getRandomString(64) }).orNull(),
  ) =
    PackageVersionV2(
      added = Random.nextLong(),
      file = getRandomFileV2(false).let { FileV1(it.name, it.sha256!!, it.size) },
  ...
  private fun getRandomManifestV2(versionCode: Long, signer: SignerV2?) =
    ManifestV2(
      versionName = getRandomString(),
      versionCode = versionCode,
      usesSdk = UsesSdkV2(minSdkVersion = Random.nextInt(), targetSdkVersion = Random.nextInt()),
      maxSdkVersion = Random.nextInt().orNull(),
      signer = signer,
      usesPermission = getRandomList { PermissionV2(getRandomString(), Random.nextInt().orNull()) },
      usesPermissionSdk23 =
        getRandomList { PermissionV2(getRandomString(), Random.nextInt().orNull()) },
