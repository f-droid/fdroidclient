...
object TestAppUtils {
  ...
  fun getRandomMetadataV2(authorName: String? = null, lastUpdated: Long? = null): MetadataV2 =
    MetadataV2(
      ...
      categories = getRandomList { getRandomString() }.orNull() ?: emptyList(),
  ...
  fun getRandomLocalizedFileListV2(): Map<String, List<FileV2>> =
    TestUtils.getRandomMap(Random.nextInt(1, 3)) {
      getRandomString() to getRandomList(Random.nextInt(1, 7)) { getRandomFileV2() }
    }

  /**
   * [Screenshots] include lists which can be ordered differently, so we need to ignore order when
   * comparing them.
   */
  ...
  private fun assertLocalizedFileListV2Equal(l1: LocalizedFileListV2?, l2: LocalizedFileListV2?) {
    if (l1 != null && l2 != null) {
      l1.keys.forEach { key -> assertEquals(l1[key]?.toSet(), l2[key]?.toSet()) }
    } else {
      assertEquals(l1, l2)
    }
  }
