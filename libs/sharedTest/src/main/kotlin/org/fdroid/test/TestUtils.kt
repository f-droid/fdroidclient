...
object TestUtils {
  ...
  fun getRandomString(length: Int = Random.nextInt(1, 128)): String =
    (1..length).map { Random.nextInt(0, charPool.size) }.map(charPool::get).joinToString("")

  fun <T> getRandomList(size: Int = Random.nextInt(0, 23), factory: () -> T): List<T> =
    if (size == 0) emptyList() else buildList { repeat(size) { add(factory()) } }

  fun <A, B> getRandomMap(size: Int = Random.nextInt(0, 23), factory: () -> Pair<A, B>): Map<A, B> =
    if (size == 0) emptyMap()
    else
      buildMap {
        repeat(size) {
  ...
  fun IndexV2.sorted(): IndexV2 =
    copy(
      packages =
        packages.toSortedMap().mapValues { entry ->
          entry.value.copy(
            metadata = entry.value.metadata.sort(),
            versions =
              entry.value.versions.mapValues {
                val pv = it.value
                pv.copy(
                  manifest =
                    pv.manifest.copy(
  ...
  fun MetadataV2.sort(): MetadataV2 =
    copy(
  ...
  fun Screenshots.sort(): Screenshots =
    copy(
      phone = phone?.sort(),
