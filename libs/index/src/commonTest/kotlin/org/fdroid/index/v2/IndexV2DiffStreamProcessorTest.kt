...
internal class IndexV2DiffStreamProcessorTest {
  ...
  @Test fun testEmptyToMin() = testDiff("$ASSET_PATH/diff-empty-min/23.json", 1)

  @Test fun testEmptyToMid() = testDiff("$ASSET_PATH/diff-empty-mid/23.json", 2)

  @Test fun testEmptyToMax() = testDiff("$ASSET_PATH/diff-empty-max/23.json", 3)

  @Test fun testMinToMid() = testDiff("$ASSET_PATH/diff-empty-mid/42.json", 2)

  @Test fun testMinToMax() = testDiff("$ASSET_PATH/diff-empty-max/42.json", 3)

  @Test fun testMidToMax() = testDiff("$ASSET_PATH/diff-empty-max/1337.json", 2)
  ...
  private class TestDiffReceiver : IndexV2DiffStreamReceiver {
    ...
    private val indexMap =
      HashMap<String, JsonElement>().apply { put("packages", JsonObject(packages)) }
    ...
    override fun receiveVersionsDiff(
      ...
      val packageV2 = HashMap<String, JsonElement>(packages[packageName]?.jsonObject ?: fail())
