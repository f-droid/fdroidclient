...
/**
 * Processes a indexV1 stream and calls the given [indexStreamReceiver] while parsing it. Attention:
 * This requires the following canonical top-level order in the JSON as produced by `fdroidserver`.
 * * repo
 * * requests
 * * apps
 * * packages
 *
 * Any other order of those elements will produce unexpected results or throw
  ...
  private inner class IndexStreamSerializer : KSerializer<IndexV1?> {
    ...
    private fun deserializeApps(decoder: JsonDecoder, index: Int): Map<String, AppData> {
      ...
      while (true) {
        ...
        val appV1 = decoder.decodeSerializableElement(descriptor, packageIndex, AppV1.serializer())
        val appV2 = appV1.toMetadataV2(null, locale)
        indexStreamReceiver.receive(appV1.packageName, appV2)
        appDataMap[appV1.packageName] =
          AppData(
    ...
    private fun readPackageMapEntry(
      ...
      while (true) {
        ...
        val suggestedVersionCode = appDataMap[packageName]?.suggestedVersionCode ?: 0
        val releaseChannels =
          if (versionCode > suggestedVersionCode) listOf(RELEASE_CHANNEL_BETA) else emptyList()
        val packageVersionV2 =
          packageVersionV1.toPackageVersionV2(
            releaseChannels = releaseChannels,
            appAntiFeatures = appDataMap[packageName]?.antiFeatures ?: emptyMap(),
            whatsNew =
              if (suggestedVersionCode == versionCode) {
