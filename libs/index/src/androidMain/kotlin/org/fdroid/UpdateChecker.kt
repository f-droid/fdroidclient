...
public class UpdateChecker(private val compatibilityChecker: CompatibilityChecker) {

  /**
   * Returns a [PackageVersion] for the given [packageInfo] that is the suggested update or null if
   * there is no suitable update in [versions].
   *
   ...
   * @param releaseChannels optional list of release channels to consider on top of stable. If this
   *   is null or empty, only versions without channel (stable) will be considered.
   * @param preferencesGetter an optional way to consider additional per app preferences
   * @param includeKnownVulnerabilities if true, versions with the [PackageInfo.getLongVersionCode]
  ...
  /**
   * Returns the [PackageVersion] that is suggested for a new installation or null if there is no
   * suitable candidate in [versions].
   *
   * @param versions a **sorted** list of [PackageVersion] with highest version code first.
   * @param preferredSigner The SHA-256 hash of the signing certificate in lower-case hex. Only
   *   versions from this signer will be considered for installation.
   * @param releaseChannels optional list of release channels to consider on top of stable. If this
   *   is null or empty, only versions without channel (stable) will be considered.
   * @param preferencesGetter an optional way to consider additional per app preferences
   */
  ...
  /**
   * Returns the [PackageVersion] that is the suggested update for the given [installedVersionCode]
   * or suggested for new installed if the given code is 0, or null if there is no suitable
   * candidate in [versions].
   *
   * @param versions a **sorted** list of [PackageVersion] with highest version code first.
   * @param allowedSignersGetter should return set of SHA-256 hashes of the signing certificates in
   *   lower-case hex. Only versions from these signers will be considered for installation. This is
   *   is null or returns null, all signers will be allowed. If the set of signers is empty, no
   *   signers will be allowed, i.e. only apps without signer.
   ...
   * @param preferencesGetter an optional way to consider additional per app preferences.
   * @param includeKnownVulnerabilities if true, versions with the [installedVersionCode] will be
   *   returned if [PackageVersion.hasKnownVulnerability] is true, even without real update.
   */
  public fun <T : PackageVersion> getUpdate(
    versions: List<T>,
  ...
  /**
   * Same as [getUpdate], but gets a list of all possible updates beginning from highest version
   * code.
   *
