...
public data class PackageV1(
  ...
  val size: Long,
  @SerialName("srcname") val srcName: String? = null,
  @SerialName("uses-permission") val usesPermission: List<PermissionV1> = emptyList(),
  @SerialName("uses-permission-sdk-23") val usesPermission23: List<PermissionV1> = emptyList(),
  val versionCode: Long? = null,
  val versionName: String,
  @SerialName("nativecode") val nativeCode: List<String>? = null,
    ...
    PackageVersionV2(
      ...
      manifest =
        ManifestV2(
          ...
          usesSdk =
            if (minSdkVersion == null && targetSdkVersion == null) null
            else
              UsesSdkV2(
      ...
      antiFeatures = appAntiFeatures + (antiFeatures?.associate { it to emptyMap() } ?: emptyMap()),
      whatsNew = whatsNew ?: emptyMap(),
    )
}

@Serializable(with = PermissionV1Serializer::class)
public data class PermissionV1(val name: String, val maxSdk: Int? = null)

internal class PermissionV1Serializer : KSerializer<PermissionV1> {
  override val descriptor =
    buildClassSerialDescriptor("PermissionV1") {
      element<String>("name")
      element<Int?>("maxSdk")
    }
