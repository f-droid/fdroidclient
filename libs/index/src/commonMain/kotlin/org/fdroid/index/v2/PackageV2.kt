...
/**
 * Like [FileV2] with the only difference that the [sha256] hash can not be null. Even in index-v1
 * this must exist, so we can use it as a primary key in the DB.
 */
@Serializable
public data class FileV1(
  ...
  @SerialName("ipfsCIDv1") override val ipfsCidV1: String? = null,
) : IndexFile {
...
@Serializable public data class UsesSdkV2(val minSdkVersion: Int, val targetSdkVersion: Int)

@Serializable
public data class SignerV2(val sha256: List<String>, val hasMultipleSigners: Boolean = false)

@Serializable public data class PermissionV2(val name: String, val maxSdkVersion: Int? = null)

@Serializable public data class FeatureV2(val name: String)
