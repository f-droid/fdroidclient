...
public interface IndexV2DiffStreamReceiver {
  ...
  /**
   * Receives one diff for a [MetadataV2] from the index stream. This is called once for each
   * package in the index diff.
   *
  ...
  /**
   * Receives the diff for all versions of the give n [packageName] as a map of versions IDs to the
   * diff [JsonObject]. This is called once for each package in the index diff (if versions have
   * changed).
   *
