...
internal class BestLocaleTest {
  ...
  fun testFallbackToEn() {
    ...
    assertEquals("en", getMap("de-AT", "de-DE", "en").getBestLocale(getLocaleList("fr-FR")))
  }

  @Test
  fun testFallbackToFirst() {
    assertEquals("de-AT", getMap("de-AT", "de-DE", "uk").getBestLocale(getLocaleList("fr-FR")))
  }
  ...
  fun testMatchLanguageAndScript() {
    ...
    assertEquals("uz-Latn", getMap("en", "uz-Cyrl", "uz-Latn").getBestLocale(getLocaleList("uz")))
    ...
    assertEquals("zh-Hant", getMap("en", "zh", "zh-Hant").getBestLocale(getLocaleList("zh-TW")))
    ...
    assertEquals("zh-Hans", getMap("en", "zh-Hant", "zh-Hans").getBestLocale(getLocaleList("zh")))
    ...
    assertEquals("de", getMap("zh", "de", "en").getBestLocale(getLocaleList("zh-HK,de")))
    ...
    assertEquals(
      "zh-Hant-HK",
      getMap("zh", "zh-Hans-CN", "zh-Hant-TW", "zh-Hant-HK").getBestLocale(getLocaleList("zh-HK")),
    )
  }
  ...
  fun testRankingPriority() {
    ...
    assertEquals(
      "en-US",
      getMap("en-Shaw-US", "en-Latn", "en", "en-US", "en-Latn-US")
        .getBestLocale(getLocaleList("en-US")),
    )

    assertEquals(
      "zh-TW",
      getMap("zh", "zh-CN", "zh-Hant", "zh-Hant-HK", "zh-TW").getBestLocale(getLocaleList("zh-TW")),
    )
    ...
    assertEquals(
      "en-Latn",
      getMap("en-Shaw-US", "en", "en-US", "en-Latn", "en-Latn-US")
        .getBestLocale(getLocaleList("en-GB")),
    )

    // finally language only if script matches
    assertEquals("de", getMap("zh", "zh-CN", "en", "de").getBestLocale(getLocaleList("zh-HK,de")))

    assertEquals("fr", getMap("zh", "en", "fr").getBestLocale(getLocaleList("en-Shaw-GB,fr")))

    assertEquals("en", getMap("fr", "en", "sr").getBestLocale(getLocaleList("sr-Latn-RS,en")))
    ...
    assertEquals("en-AR", getMap("en-AR", "en-GB", "en-US").getBestLocale(getLocaleList("en-NL")))
  ...
  fun testListConversion() {
    ...
    assertEquals(
      "zh-TW",
      getMap("en-US", "zh-CN", "zh-HK", "zh-TW")
    ...
    assertEquals(
      "fr-FR",
      getMap("en-US", "de-DE", "es-ES", "fr-FR", "it-IT").getBestLocale(getLocaleList("fr-CH")),
    )

    // https://developer.android.com/guide/topics/resources/multilingual-support#t-2d-choice
    assertEquals(
      "it-IT",
      getMap("en-US", "de-DE", "es-ES", "it-IT").getBestLocale(getLocaleList("fr-CH,it-CH")),
    )
  }
  ...
  fun testInvalidLocales() {
    ...
    assertEquals(
      "en-US",
      getMap("foo-Bar", "324;kfj4297h4c2oj", "2342142143", "de_DE", "#$%#!$^#&^%#*", "en-US")
